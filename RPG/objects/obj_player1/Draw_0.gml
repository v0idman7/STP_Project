...
if (input_fight) && (weapon_sword) && (weapon_ind = 1){ //если выполняем удар мечём
...
attack1 = 0}
if (input_fight) && (weapon_bow) && (weapon_ind = 2){ //если используем лук
	switch(pos){ //от текущеё позиции выбираем какую анимацию выполнить
...
if ((weapon_sword || weapon_bow) && attack1 = 29 && attack2 = 64){ //отрисовка оружия при ходьбе
	switch(pos){
	...
	switch(weapon_ind){
		case 1: if weapon_sword draw_sprite_part(spr_longsword_male, 0, floor(0)*192, y_fr*192, 192, 192, xx-64, yy-64); break;
		case 2: if weapon_bow {draw_sprite_part(spr_bow, 0, floor(0)*frame_size, y_fr*frame_size, frame_size, frame_size, xx, yy);
